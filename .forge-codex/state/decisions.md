...
- Selected option: generate the allowed per-architecture daemon CodeDirectory hash set during the signed build and bind it into each main caller executable's mapped code or a kernel-attested entitlement. The client must precompile a daemon requirement conjoined with that exact `cdhash` set before activating NSXPC. The app, installed manager, installed raw CLI, and standalone CLI must share one verified app/helper/plist relationship and exercise it as distinct signed processes. Path SHA-256 may remain diagnostic only.
...
- Current authority: not implemented or qualified in this checkpoint. `FC-PRIVILEGED-CALLER-IDENTITY-001`, `FC-PRIVILEGED-SERVICE-LIFECYCLE-001`, `FC-FILESYSTEM-PATH-TOCTOU-001`, E2, P10, G10, and G12 remain open.
