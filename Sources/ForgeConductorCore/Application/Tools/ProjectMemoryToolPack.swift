...
public struct ProjectMemoryToolPack: ToolPackHandling {
    ...
    public func handle(
        ...
        do {
            try validateDeadline(arguments)
            let payload: [String: Any]
            switch name {
            case "project_memory.initialize": payload = try initialize(arguments, service: app.projectMemory)
            case "project_memory.remember": payload = try remember(arguments, service: app.projectMemory)
            case "project_memory.remember_batch": payload = try rememberBatch(arguments, service: app.projectMemory)
            case "project_memory.search": payload = try search(arguments, service: app.projectMemory)
            case "project_memory.get": payload = try get(arguments, service: app.projectMemory)
            case "project_memory.update": payload = try update(arguments, service: app.projectMemory)
            case "project_memory.forget": payload = try forget(arguments, service: app.projectMemory)
            case "project_memory.list_recent": payload = try listRecent(arguments, service: app.projectMemory)
            case "project_memory.link": payload = try link(arguments, service: app.projectMemory)
            case "project_memory.export": payload = try app.projectMemory.export(projectID: try projectID(arguments))
            case "project_memory.import": payload = try importArtifact(arguments, service: app.projectMemory)
            case "project_memory.status": payload = try app.projectMemory.status(projectID: try projectID(arguments))
    ...
    private func initialize(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        ...
        return try service.initialize(
            ...
            repositoryIdentity: string(arguments, "repository_identity")
        )
    }

    private func remember(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        try service.remember(projectID: projectID(arguments), write: write(arguments))
    }

    private func rememberBatch(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        ...
        return try service.rememberBatch(projectID: projectID(arguments), writes: items.map(write))
    }

    private func search(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
    ...
    private func get(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        var ids = strings(arguments["ids"])
        if let id = string(arguments, "id") { ids.insert(id, at: 0) }
        return try service.get(projectID: projectID(arguments), ids: Array(Set(ids)).sorted(), includeBody: boolean(arguments, "include_body") ?? false)
    }

    private func update(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        ...
        return try service.update(
            ...
            tags: arguments["tags"] == nil ? nil : strings(arguments["tags"])
        )
    }

    private func forget(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        guard let id = string(arguments, "id") else { throw ProjectMemoryError.invalidRequest("id is required") }
        return try service.forget(projectID: projectID(arguments), id: id)
    }

    private func listRecent(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
    ...
    private func link(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
        ...
        return try service.link(projectID: projectID(arguments), sourceID: source, targetID: target, relation: relation)
    }

    private func importArtifact(_ arguments: [String: Any], service: ProjectMemoryService) throws -> [String: Any] {
    ...
    private func validateDeadline(_ arguments: [String: Any]) throws {
        guard let deadline = integer(arguments, "deadline_ms") else { return }
