...
final class ContinuityIngressAcceptanceTests: XCTestCase {
    ...
    func testOldVersionTwoMigrationRetainsRunsBindingsAndForeignKeysWithVerifiedBackup() async throws {
        try await withFixture { fixture in
            ...
            XCTAssertEqual(try AcceptanceSQLite.integer(fixture.database, "SELECT COUNT(*) FROM forge_migration_receipts"), 8)
    ...
    func testFullAppLegacyUpgradeAndReopenPreserveIndependentMigrationLineages() async throws {
        ...
        XCTAssertEqual(ingress.sourceVersion, 8)
        XCTAssertEqual(ingress.targetVersion, 9)
        XCTAssertEqual(try AcceptanceSQLite.integer(database, "SELECT COUNT(*) FROM forge_migration_receipts"), 9)
