...
final class ContinuityIngressAcceptanceTests: XCTestCase {
    ...
    func testOldVersionTwoMigrationRetainsRunsBindingsAndForeignKeysWithVerifiedBackup() async throws {
        try await withFixture { fixture in
            ...
            XCTAssertEqual(try AcceptanceSQLite.integer(fixture.database, "SELECT COUNT(*) FROM forge_migration_receipts"), 9)
            ...
            XCTAssertEqual(manifest.sourceVersion, 9)
            XCTAssertEqual(manifest.targetVersion, 10)
    ...
    func testFullAppLegacyUpgradeAndReopenPreserveIndependentMigrationLineages() async throws {
        ...
        XCTAssertEqual(ingress.sourceVersion, 9)
        XCTAssertEqual(ingress.targetVersion, 10)
        XCTAssertEqual(try AcceptanceSQLite.integer(database, "SELECT COUNT(*) FROM forge_migration_receipts"), 10)
