...
final class NativeSourcePressureJournalTests: XCTestCase {
    ...
    func testSchemaEightUpgradePreservesLegacyIntentAndVerifiedBackup() async throws {
        try await withFixture { f in
            ...
            try PressureSQL.execute(f.database, "ALTER TABLE native_task_capabilities DROP COLUMN profile_version")
            ...
            do {
                ...
                XCTAssertEqual(try PressureSQL.value(backup, "SELECT COUNT(*) FROM pragma_table_xinfo('native_source_provider_turns') WHERE name LIKE 'pressure_%'"), "3")
                XCTAssertEqual(try PressureSQL.value(backup, "SELECT COUNT(*) FROM pragma_table_xinfo('native_task_capabilities') WHERE name='profile_version'"), "0")
                let capabilityEightBackup = f.database.deletingPathExtension().appendingPathExtension("pre-ingress-capability-v8.sqlite3")
                XCTAssertEqual(try PressureSQL.value(capabilityEightBackup, "SELECT intent_json FROM native_source_provider_turns"), String(decoding: legacy, as: UTF8.self))
                XCTAssertEqual(try PressureSQL.value(capabilityEightBackup, "SELECT COUNT(*) FROM pragma_table_xinfo('native_source_provider_turns') WHERE name LIKE 'pressure_%'"), "0")
