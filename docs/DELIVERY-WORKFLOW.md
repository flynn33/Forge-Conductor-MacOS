...
The owner authorizes continuous completion toward the [functional development build](FUNCTIONAL-DEVELOPMENT-BUILD.md). Work remains small, coherent and tested, but a phase or PR boundary does not terminate independent authorized work. The canonical record is [ROADMAP.md](../ROADMAP.md); its phase/milestone rows track outcomes and do not dispatch tasks. Historical plans remain evidence, not competing instructions.
...
Confirm the exact repository and current branch. Preserve unrelated or uncommitted work. At engagement entry and after an authorized merge, fetch the verified remote, safely fast-forward clean local main, and continue or create the coherent completion branch. Never reset/stash/discard owner work, force-push, or make product-file commits only on GitHub.
...
Commit tested changes locally with existing owner identity/signing, stage only explicit intended paths, then push the completion branch. Create or update one reviewable PR to main with the actual exposed GitHub tool. Preserve contribution and identity rules. An unavailable publishing or commit-signing capability blocks publication, not local build/test/implementation work, and is not an invitation to create credentials or install an API wrapper.
...
Owner review controls merge acceptance. Do not merge, enable auto-merge, or repeatedly poll for approval. Continue independent authorized completion work while review or CI is pending. GitHub merge state determines whether linked work is accepted. A roadmap row labelled “ready in linked PR” is a proposal until that PR is merged; no static checkbox overrides the actual merge state.
...
When an authorized merge is observed, confirm the exact PR merged into main. Preserve local work, fetch, and fast-forward clean local main. Confirm local main matches remote main. Check the canonical workspace and any newly incorporated source/resource/build changes. No separate synchronization-only assignment is required.
...
This step does not authorize protected merge or publication. No constant polling, custom synchronization script, or second checkout is needed.
